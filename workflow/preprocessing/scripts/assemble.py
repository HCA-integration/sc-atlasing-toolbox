...
def assemble_zarr(file, file_type, files_to_keep):
    file = Path(file)

    if file_type == 'counts':
        logging.debug('add raw counts')
        files_to_keep.append((file / 'X', output_file / 'layers' / 'counts'))
    elif file_type == 'normalize':
        logging.debug('add normalised counts')
        files_to_keep.append((file / 'X', output_file / 'layers' / 'normcounts'))
        files_to_keep.append((file / 'X', output_file / 'X'))
        files_to_keep.append((file / 'uns' / 'log1p', output_file / 'uns' / 'log1p'))
        files_to_keep.append((file / 'uns' / 'preprocessing' / 'log-transformed', output_file / 'uns' / 'preprocessing' / 'log-transformed'))
        files_to_keep.append((file / 'uns' / 'preprocessing' / 'normalization', output_file / 'uns' / 'preprocessing' / 'normalization'))
    elif file_type == 'highly_variable_genes':
        logging.debug('add highly variable gene info')
        files_to_keep.append((file / 'var', output_file / 'var'))
        files_to_keep.append((file / 'uns' / 'preprocessing' / 'highly_variable_genes', output_file / 'uns' / 'preprocessing' / 'highly_variable_genes'))
    elif file_type == 'pca':
        logging.debug('add PCA')
        files_to_keep.append((file / 'obsm' / 'X_pca', output_file / 'obsm' / 'X_pca'))
        files_to_keep.append((file / 'uns' / 'pca', output_file / 'uns' / 'pca'))
        files_to_keep.append((file / 'varm' / 'PCs', output_file / 'varm' / 'PCs'))
    elif file_type == 'neighbors':
        logging.debug('add neighbors')
        files_to_keep.append((file / 'obsp' / 'connectivities', output_file / 'obsp' / 'connectivities'))
        files_to_keep.append((file / 'obsp' / 'distances', output_file / 'obsp' / 'distances'))
        files_to_keep.append((file / 'uns' / 'neighbors', output_file / 'uns' / 'neighbors'))
    elif file_type == 'umap':
        logging.debug('add UMAP')
        files_to_keep.append((file / 'obsm' / 'X_umap', output_file / 'obsm' / 'X_umap'))
    else:
        ValueError(f'Unknown file type {file_type}')
