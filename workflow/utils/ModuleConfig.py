...
class ModuleConfig:
    ...
    def __init__(self, module_name: str, config: dict, parameters: pd.DataFrame = None):
        ...
        default_datasets = config['defaults'].get('datasets', all_datasets.keys())
        self.datasets = {
    ...
    def set_input_file_per_dataset(self, dataset, digest_size=5):
    ...
    def get_output_files(self, pattern, exclude: list = None):
        if exclude is None:
            exclude = []
    ...
    def get_input_file(self, dataset, file_id):
        return get_input_files_per_dataset(dataset)[file_id]
